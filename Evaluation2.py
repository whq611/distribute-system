...
for node in range(nodenums):
    with open("s2/node" + str(node) +".txt","r") as fs:
        for line in fs:
            l.append(line)
...
dy.set(xlabel='time(second)', ylabel='delay(second)', title='Evaluation Delay S2')


bd = plt.subplot(212, sharex = dy)
bd.plot(timeRange, pd.DataFrame(Bandwidths).T.sum(), marker='o')
bd.legend(['Sum'])
bd.set(xlabel='time(second)', ylabel='bandwidth(byte/second)', title='Evaluation Bandwidth S2')
